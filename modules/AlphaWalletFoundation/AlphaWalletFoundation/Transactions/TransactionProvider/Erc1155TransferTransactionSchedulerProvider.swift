...
protocol Erc1155TransferTransactionSchedulerDelegate: AnyObject {
    func didReceiveResponse(_ response: Result<[Transaction], PromiseError>, in provider: Erc1155TransferTransactionSchedulerProvider)
}
...
final class Erc1155TransferTransactionSchedulerProvider: SchedulerProvider {
    ...
    private func fetchPublisher() -> AnyPublisher<Void, PromiseError> {
        ...
        return networking
            .erc1155TokenTransferTransaction(walletAddress: session.account.address, pagination: pagination)
