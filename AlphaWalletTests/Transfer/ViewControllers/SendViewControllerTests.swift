...
class SendViewControllerTests: XCTestCase {
    ...
    private let token = Token(contract: AlphaWallet.Address.make(), server: .main, value: "0", type: .nativeCryptocurrency)
    ...
    func testUpdateNativeCryptoBalance() {
        let token = Token(contract: AlphaWallet.Address.make(), server: .main, value: "0", type: .nativeCryptocurrency)
