...
class TransactionConfiguratorTests: XCTestCase {
    ...
    func testMinGasPrice() throws {
        let analytics = FakeAnalyticsService()
        let configurator = try TransactionConfigurator(session: .make(), analytics: analytics, transaction: .make(gasPrice: BigUInt(1000000000)))
        XCTAssertEqual(GasPriceConfiguration.minPrice, configurator.currentConfiguration.gasPrice)
    }

    func testMaxGasPrice() throws {
        let analytics = FakeAnalyticsService()
        let configurator = try TransactionConfigurator(session: .make(), analytics: analytics, transaction: .make(gasPrice: BigUInt(990000000000)))
