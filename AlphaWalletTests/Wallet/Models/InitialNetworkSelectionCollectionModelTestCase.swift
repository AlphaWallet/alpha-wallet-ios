...
class InitialNetworkSelectionCollectionModelTestCase: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        model.addSelected(servers: [RPCServer.main, RPCServer.sepolia, RPCServer.avalanche])
    }
    ...
    func testSelected() {
        XCTAssertTrue(model.selected == [.main, .sepolia, .avalanche], "\(model.selected)")
    }

    func testAddSelected() {
        model.addSelected(server: .arbitrum)
        XCTAssertTrue(model.selected == [.main, .sepolia, .avalanche, .arbitrum], "\(model.selected)")
    }

    func testRemoveSelected() {
        model.removeSelected(server: .sepolia)
