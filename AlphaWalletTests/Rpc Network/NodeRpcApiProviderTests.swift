...
class NodeRpcApiProviderTestCase: XCTestCase {
    ...
    private lazy var provider = NodeRpcApiProvider(
        rpcApiProvider: BaseRpcApiProvider.make(networkService: networkService),
        server: .main,
    ...
    func testSwitchRpcUrlWithFailureCompletionForPromises() {
        ...
        provider
            .dataTaskPromise(FakeBlockNumberRequest())
            .done { _ in }
    ...
    func testSwitchRpcUrlWithFailureCompletionForPublisher() {
        ...
        provider
            .dataTaskPublisher(FakeBlockNumberRequest())
