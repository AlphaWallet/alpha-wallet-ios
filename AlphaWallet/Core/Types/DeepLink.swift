...
enum DeepLink {
    ...
    init?(url: URL, supportedServers: [RPCServer] = [.main]) {
        ...
        } else if let eip681Url = Self.functional.extractEmbeddedEip681Url(in: url, supportedServers: supportedServers) {
            NSLog("xxx DeepLink init EIP681: \(eip681Url.absoluteString)")
            self  = .eip681(url: eip681Url)
        } else if let (wcUrl, source) = Self.functional.extractEmbeddedWalletConnectUrl(in: url) {
...
extension DeepLink.functional {
    ...
    static func extractEmbeddedEip681Url(in url: URL, supportedServers: [RPCServer]) -> URL? {
    ...
    static func extractEmbeddedWalletConnectUrl(in url: URL) -> (url: AlphaWallet.WalletConnect.ConnectionUrl, source: DeepLink.WalletConnectSource)? {
