...
class FilterTokensCoordinator {
    ...
    func loansFinancesTokens(tokens: [TokenObject]) -> [TokenObject] {
        return tokens.filter { Constants.loansContracts.contains($0.contractAddress) }
    }

    func depositFinancesTokens(tokens: [TokenObject]) -> [TokenObject] {
        return tokens.filter { Constants.depositContracts.contains($0.contractAddress) }
    }
