...
struct SingleServerWalletConnectSession: Codable, SessionIdentifiable {
...
class WalletConnectV1Provider: WalletConnectServerType {
    ...
    private var server: Server {
        if let server = WalletConnectV1Provider.server {
            return server
        } else {
            let server = Server(delegate: self)
            WalletConnectV1Provider.server = server
...
//        self.sessions.value = server.openSessions().map { session -> SingleServerWalletConnectSession in
//            let server = session.dAppInfo.chainId.flatMap({ RPCServer(chainID: $0) }) ?? Config().anyEnabledServer()
//            return .init(session: session, server: server)
//        }
    ...
    func updateSession(session: AlphaWallet.WalletConnect.Session, servers: [RPCServer]) throws {
        guard let index = storage.value.firstIndex(where: { $0.identifier == session.identifier }), let server = servers.first else { return }
    ...
    func reconnectSession(session: AlphaWallet.WalletConnect.Session) throws {
        guard let nativeSession = storage.value.first(where: { $0.identifier == session.identifier }) else { return }
    ...
    func disconnectSession(sessions: [NFDSession]) throws {
        for each in sessions {
            guard let session = storage.value.first(where: { $0.identifier == each.session.identifier }) else { continue }
    ...
    func hasConnectedSession(session: AlphaWallet.WalletConnect.Session) -> Bool {
        guard let nativeSession = storage.value.first(where: { $0.identifier == session.identifier }) else { return false }
...
extension WalletConnectV1Provider: WalletConnectV1ServerRequestHandlerDelegate {
    ...
    func handler(_ handler: RequestHandlerToAvoidMemoryLeak, request: WalletConnectV1Request) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            guard let session = strongSelf.storage.value.first(where: { $0.session.url == request.url }) else {
...
extension WalletConnectV1Provider: ServerDelegate {
    ...
    private func removeSession(for url: WalletConnectV1URL) {
        if let index = storage.value.firstIndex(where: { $0.session.url.absoluteString == url.absoluteString }) {
        ...
        DispatchQueue.main.async {
            if let delegate = self.delegate {
                ...
                delegate.server(self, shouldConnectFor: sessionProposal) { [weak self] choice in
                    ...
                    if let index = strongSelf.storage.value.firstIndex(where: { $0.identifier == session.identifier }) {
    ...
    @discardableResult private func addOrUpdateSession(session: Session) -> SingleServerWalletConnectSession {
        ...
        if let index = storage.value.firstIndex(where: { $0.identifier == session.identifier }) {
