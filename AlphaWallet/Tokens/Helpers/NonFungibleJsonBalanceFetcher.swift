...
class NonFungibleJsonBalanceFetcher {
    ...
    init(server: RPCServer, tokensDataStore: TokensDataStore, queue: DispatchQueue) {
        self.server = server
    ...
    func fetchNonFungibleJson(forTokenId tokenId: String, tokenType: TokenType, address: AlphaWallet.Address, enjinTokens: EnjinTokenIdsToSemiFungibles) -> Guarantee<NonFungibleBalanceAndItsSource<JsonString>> {
        firstly {
