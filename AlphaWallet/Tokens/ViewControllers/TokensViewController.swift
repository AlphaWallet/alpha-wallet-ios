...
class TokensViewController: UIViewController {
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.registerHeaderFooterView(FilterTokensTableHeaderView.self)
        tableView.registerHeaderFooterView(AddHideTokensTableHeaderView.self)
        tableView.registerHeaderFooterView(TokensTableHeaderView.self)
    ...
    private lazy var collectiblesCollectionView: UICollectionView = {
        ...
        //NOTE: set it to false to make the same behaviour as for UITableView, hide header view, when it out of bounds
        ...
        collectionView.registerSupplementaryView(FilterTokensCollectionHeaderView.self, of: UICollectionView.elementKindSectionHeader)
    ...
    private func contractsForCollectiblesFromViewModel() -> [AlphaWallet.Address] {
        ...
        for i in (0..<viewModel.numberOfItems(section: 1)) {
...
extension TokensViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        case .filter:
            let view: FilterTokensTableHeaderView = tableView.dequeueReusableHeaderFooterView()
            view.filterView = tableViewFilterView
            return view
        case .addHideToken:
            let view: AddHideTokensTableHeaderView = tableView.dequeueReusableHeaderFooterView()
            view.delegate = self
            view.configure()
            return view
        case .deposit:
            let view: TokensTableHeaderView = tableView.dequeueReusableHeaderFooterView()
            view.configure(viewModel: .init(title: viewModel.depositTitleLabel))
            return view
        case .loans:
            let view: TokensTableHeaderView = tableView.dequeueReusableHeaderFooterView()
            view.configure(viewModel: .init(title: viewModel.loansTitleLabel))
            return view
        default:
            return nil
        }
...
extension TokensViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        let header: FilterTokensCollectionHeaderView = collectionView.dequeueReusableSupplementaryView(ofKind: kind, for: indexPath)
