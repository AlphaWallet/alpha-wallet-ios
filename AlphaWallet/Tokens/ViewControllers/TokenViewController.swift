...
class TokensViewController: UIViewController, TokenVerifiableStatusViewController {
    ...
    let header = TokensViewControllerHeader()
    ...
    init(config: Config, tokenObject: TokenObject, account: Wallet, session: WalletSession, tokensStorage: TokensDataStore, viewModel: TokensViewModel) {
        self.config = config
        ...
        tableView.register(TokenTableViewCellWithoutCheckbox.self, forCellReuseIdentifier: TokenTableViewCellWithoutCheckbox.identifier)
        ...
        redeemButton.setTitle(R.string.localizable.aWalletTokenTokenRedeemButtonTitle(), for: .normal)
        redeemButton.addTarget(self, action: #selector(redeem), for: .touchUpInside)

        sellButton.setTitle(R.string.localizable.aWalletTokenTokenSellButtonTitle(), for: .normal)
        sellButton.addTarget(self, action: #selector(sell), for: .touchUpInside)

        transferButton.setTitle(R.string.localizable.aWalletTokenTokenTransferButtonTitle(), for: .normal)
    ...
    func configure(viewModel newViewModel: TokensViewModel? = nil) {
...
extension TokensViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .otherNonFungibleToken:
            let cell = tableView.dequeueReusableCell(withIdentifier: TokenTableViewCellWithoutCheckbox.identifier, for: indexPath) as! TokenTableViewCellWithoutCheckbox
            cell.configure(viewModel: .init(TokenHolder: TokenHolder))
            return cell
        }
    }
...
extension TokensViewController: BaseTokenListFormatTableViewCellDelegate {
