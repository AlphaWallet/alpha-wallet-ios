...
class ButtonsBar: UIView {
    ...
    func configure(_ newConfiguration: ButtonsBarConfiguration? = nil) {
        ...
        for (index, buttonType) in configuration.buttonViewModels.enumerated() {
            ...
            view.childView.isHidden = configuration.shouldHideButton(at: index)
            switch buttonType {
            case .green:
                setup(viewModel: .greenButton, view: view)
            case .white:
                setup(viewModel: .whiteButton, view: view)
            }
        }
    ...
    @objc private func optionsButtonTapped(sender: UIButton) {
        let buttons = self.buttons.enumerated().filter { configuration.shouldHideButton(at: $0.offset) }.map { $0.element }
