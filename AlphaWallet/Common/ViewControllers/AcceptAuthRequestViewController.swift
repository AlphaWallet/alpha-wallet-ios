...
protocol AcceptAuthRequestViewControllerDelegate: AnyObject {
...
class AcceptAuthRequestViewController: UIViewController {
    ...
    private (set) var viewModel: AcceptAuthRequestViewModel
    ...
    weak var delegate: AcceptAuthRequestViewControllerDelegate?

    init(viewModel: AcceptAuthRequestViewModel) {
    ...
    private func configure(for viewModel: AcceptAuthRequestViewModel) {
        buttonsBar.configure(.custom(types: [.primary, .secondary]))
        headerView.iconImageView.setImage(url: nil, placeholder: R.image.walletConnectIcon())
...
extension AcceptAuthRequestViewController: TransactionConfirmationHeaderViewDelegate {
