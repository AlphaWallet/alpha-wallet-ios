...
class ConfigureTransactionViewController: UIViewController {
    ...
    private let contentInsets: UIEdgeInsets = {
        let bottomInset: CGFloat = ScreenChecker().isNarrowScreen ? 16 : 20

        return .init(top: 5, left: 16, bottom: bottomInset, right: 16)
    }()
            ...
            case .custom:
                ...
                case .field(let fieldType):
                    ...
                    case .nonce:
                        ...
                        views += [nonceTextField.defaultLayout(edgeInsets: contentInsets), UIView.separator()]
                    case .totalFee:
                        totalFeeTextField.configure(viewModel: viewModel.totalFeeViewModel)

                        views += [totalFeeTextField.defaultLayout(edgeInsets: contentInsets), UIView.separator()]
                    case .transactionData:
                        dataTextField.configure(viewModel: viewModel.dataViewModel)

                        views += [dataTextField.defaultLayout(edgeInsets: contentInsets), UIView.separator()]
                    }
                }
            }
        }
