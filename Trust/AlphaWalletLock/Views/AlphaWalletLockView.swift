...
//Duplicated from AlphaWalletLockView.swift for easier upstream merging
class AlphaWalletLockView: UIView {
	var characterView = UIStackView()
	var lockTitle = UILabel()
	var model: LockViewModel!
	var characters: [AlphaWalletPasscodeCharacterView]!
	...
	private func passcodeCharacters() -> [AlphaWalletPasscodeCharacterView] {
		var characters = [AlphaWalletPasscodeCharacterView]()
		for _ in 0..<model.charCount() {
			let passcodeCharacterView = AlphaWalletPasscodeCharacterView()
