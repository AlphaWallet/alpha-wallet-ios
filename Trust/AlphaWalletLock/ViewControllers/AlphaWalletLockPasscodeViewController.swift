...
//Duplicated from LockPasscodeViewController.swift for easier upstream merging
class AlphaWalletLockPasscodeViewController: UIViewController {
	var willFinishWithResult: ((_ success: Bool) -> Void)?
	let model: LockViewModel
	var lockView: AlphaWalletLockView!
	let lock = Lock()
	...
	private func configureLockView() {
		lockView = AlphaWalletLockView(model)
...
extension AlphaWalletLockPasscodeViewController: UITextFieldDelegate {
