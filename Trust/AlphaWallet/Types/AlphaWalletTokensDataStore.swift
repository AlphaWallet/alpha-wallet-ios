...
protocol AlphaWalletTokensDataStoreDelegate: class {
...
class AlphaWalletTokensDataStore {
    ...
    let config: Config
    let web3: Web3Swift
    weak var delegate: AlphaWalletTokensDataStoreDelegate?
    let realm: Realm
    ...
    private func addEthToken() {
        ...
        let etherToken = AlphaWalletTokensDataStore.etherToken(for: config)
    ...
    func refreshETHBalance() {
        self.getBalanceCoordinator.getEthBalance(for: self.account.address) {  [weak self] result in
            ...
            switch result {
            case .success(let balance):
                let etherToken = AlphaWalletTokensDataStore.etherToken(for: self.config)
    ...
    func updateDelegate() {
        ...
        let tokensViewModel = AlphaWalletTokensViewModel( tokens: enabledObject, tickers: tickers )
